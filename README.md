...
le code fonctionne sur pyhton et nécessite les dépendances présentes dans le fichier `requirements.txt`. Pour rappel, les dépendances nécessaires sont :
...
*`requirements.txt` s'exécute dans. (commmande : pip install -r .\requirements.txt)* 
...
Ils seront enregistrés dans un fichier CSV dans le dossier [Victimes](CSV/Stats/Victimes) avec leur nom respectifs.

Les noms de domaines des noms de victimes seront extraits et enregistrés dans le fichier `Victimes_Domaines.csv`.

Les victimes considérées comme point de vente seront affichées dans le fichier `Victimes_Retail.csv`.
...
Le script `Victimes_Retail.py` cherche dans le fichiers`Victimes_Globales.csv` les victimes qui sont des points de vente et enregistre le résultat dans le fichier`Victimes_Retail.csv` dans le dossier [Victimes_Retail](CSV/Stats/Victimes).
...
Le script `Excel.py` se charge de récupérer les fichiers de stats, mais également les fichiers `Groupes.csv`, `Attaques.csv`, `Victimes.csv` et `Presse_Globale.csv` pour les mettre dans le fichier Excel `Veille.xlsx` sous forme de tableau. Il y aura une feuille par fichiers.
...
Si nous voulons le lancer, il faudra taper dans un terminal Powershell `python -m http.server` ou exécuter le Script `Lancement_Serveur.ps1` qui ouvrira le bon port<sup>3</sup>.
