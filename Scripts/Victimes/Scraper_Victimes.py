...
def enregistrer_csv(data, chemin_csv):
    try:
        ...
        with open(chemin_csv, mode='w', newline='', encoding='utf-8') as fichier_csv:
            ...
            for ligne in data:
                writer.writerow([ligne['date'], ligne['victim'], ligne['domain'], ligne['country'],
                                 ligne['summary'], ligne['title'], ligne['url']])
        
        print(f"Les données ont été enregistrées avec succès dans {chemin_csv}")
